...
export default function Categories() {
  ...
  return (
    <div className="bg-white text-slate-800 dark:bg-slate-950 dark:text-slate-100">
      ...
      >
        <PageHeroBreadcrumb delay={2} items={[{ label: 'Home', to: '/' }, { label: 'Categories' }]} />
      </PageHero>
            ...
            {categoryRoots.map((c) => {
              ...
              return (
                <Link
                  ...
                  className="group flex flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:border-[#A36783]/40 hover:shadow-lg hover:shadow-[#A36783]/10 dark:border-slate-700 dark:bg-slate-900/40"
                >
                  <div className="relative aspect-[16/10] overflow-hidden bg-slate-100 dark:bg-slate-800">
                    ...
                    <p className="absolute left-5 top-5 rounded-full bg-white/90 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-[#A36783] shadow-sm backdrop-blur-sm dark:bg-slate-900/85">
                      Line
                    </p>
                  </div>
                  <div className="flex flex-1 flex-col p-6 md:p-8">
                    <h2 className="text-xl font-bold text-slate-900 group-hover:text-[#A36783] dark:text-white dark:group-hover:text-[#c9a0b5]">
                      {c.name}
                    </h2>
                    ...
                    <span className="mt-8 text-sm font-bold uppercase tracking-wider text-[#A36783]">Explore →</span>
                  </div>
                </Link>
              )
            })}
